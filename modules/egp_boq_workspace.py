...
def render_boq_workspace():
    # 1. Extract Session Security parameters from your main router configuration layout
    ...
    with tab1:
                    ...
                    if st.button("Ingest File & Run Fuzzy Match Alignment"):
                        if new_tid and new_file:
                            ...
                            for _, row in df_in.iterrows():
                                ...
                                # Apply data sanitizer pipelines
        ...
        can_edit = current_role in ['admin', 'system_admin', 'company_admin'] and workflow_status != 'Approved'
        
        # Display data editor
        edited_grid = st.data_editor(
            df_items,
            hide_index=True,
            disabled=['id', 'item_no', 'group_name', 'item_code', 'description', 'unit', 'quantity'] if not can_edit else ['id', 'item_no', 'group_name', 'item_code', 'description', 'unit'],
        ...
        if can_edit:
            ...
            with col_b3:
                if workflow_status == 'Draft' and st.button("📤 Escalate to Executive Review Cycle"):
                    conn = sqlite3.connect(DB_PATH)
                    cursor = conn.cursor()
                    cursor.execute(
        ...
        # ==========================================
        # 📈 TAB 2: OVERHEADS & ANALYTICS (Inside Tab1)
        # ==========================================
        with tab2:
            st.subheader("⚙️ Global Scale Factor Injectors")

            if not can_edit:
                st.warning("🔒 Overhead manipulation operations are restricted during active review evaluations.")
            else:
            ...
            k1.metric("Gross Projected Cost", f"BDT {gross_base_cost:,.2f}")
            k2.metric("Target Budget Cap Limit", f"BDT {budget_limit:,.2f}" if budget_limit > 0 else "Unspecified")
            k3.metric("Project Variance Buffer", f"BDT {(budget_limit - gross_base_cost):,.2f}" if budget_limit > 0 else "N/A")
        ...
        with tab3:
            ...
            with col_cp1:
                with st.form("comp_form", clear_on_submit=True):
                    c_name = st.text_input("Competitor Company / Joint Venture Identity Name")
        ...
        with tab4:
            ...
            if df_arch.empty:
                st.info("No recorded historical tender profiles stored inside the database repository archives.")
            else:
            ...
            if not app_list:
                st.warning("🔒 Package Export Lock: Generation operations remain restricted until a package receives an Approved executive signature.")
            else:
