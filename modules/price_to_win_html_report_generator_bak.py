...
<head>
    ...
    <title>TenderAI - Price-to-Win Analysis Report</title>
    <style>
...
<body>
    <div class="report-container">
        <div class="header">
            <h1>🤖 TenderAI | Price-to-Win Analysis Report</h1>
        ...
        <div class="content">
            ...
            <div class="section">
                ...
                <div class="info-grid">
                    ...
                    <div class="info-item"><div class="info-label">PPR 2025</div><div class="info-value">{compliance_badge}</div></div>
                </div>
            </div>
                ...
                <div class="recommendation-box">
                    <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; margin-bottom: 15px;">
                        <div>
                            ...
                            <span style="font-size: 13px; color: #065f46;">Based on {competitor_count} competitor scenarios</span>
                        </div>
    ...
    def _generate_scenario_table(self, scenarios):
        ...
        for s in scenarios:
            ...
            <tr>
                ...
                <td style="text-align: center;">
                    <div class="progress-bar"><div class="progress-fill" style="width: {win_prob*100:.0f}%;"></div></div>
                    <span style="font-size: 12px;">{win_prob*100:.0f}%</span>
                </td>
                <td style="text-align: right;">{s.get('expected_profit', 0):,.3f}</td>
                <td style="text-align: center;">
        ...
        <table>
            <thead>
                <tr><th>#</th><th>Competitors</th><th>Optimal Bid (BDT)</th><th>Bid Ratio</th><th>Win Probability</th><th>Expected Profit</th><th>Risk</th></tr>
            </thead>
    ...
    def _generate_detailed_scenarios(self, scenarios_full):
        ...
        html = ''
        for scenario in scenarios_full[:3]:  # Show top 3 scenarios
            comp_bids = scenario.get('competitor_bids', [])
            stats = scenario.get('competitor_stats', {})
            
            bid_rows = ''
            for i, bid in enumerate(comp_bids[:10], 1):
                bid_rows += f'<tr><td>{i}</td><td>Competitor {i}</td><td style="text-align: right;">{bid:,.3f}</td></tr>'
            
            html += f'''
            <div style="background: #f8fafc; padding: 20px; border-radius: 12px; margin-bottom: 20px;">
                <h4>Scenario {scenario['scenario_id']}: {scenario['num_competitors']} Competitors</h4>
        ...
        if len(scenarios_full) > 3:
            html += f'<p><strong>Plus {len(scenarios_full) - 3} additional scenarios</strong> (full details in application)</p>'
    ...
    def _generate_competitor_table(self, competitor_bids, official_estimate):
        ...
        for i, comp in enumerate(competitor_bids[:15], 1):
            ...
            rows += f'<tr><td>{i}</td><td>Competitor {i}</td><td style="text-align: right;">{bid:,.3f}</td><td style="text-align: center;">{pct:.2f}%</td></tr>'
    ...
    def _generate_calculation_table(self, data, official_estimate):
        ...
        steps = [
            ...
            (2, 'NPPI Factor', '28-day market average', f'{nppi_factor:.4f}', 'Index Factor'),
