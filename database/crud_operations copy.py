...
class DatabaseCRUD:
    ...
    def update_company_subscription(self, company_id: int, plan: str, 
        ...
        with self.get_connection() as conn:
            ...
            plan_limits = {
                'free': {'limit': 5, 'price': 0, 'max_boq': 5, 'max_bid': 5},
                'basic': {'limit': 30, 'price': 4999, 'max_boq': 30, 'max_bid': 30},
                'professional': {'limit': -1, 'price': 14999, 'max_boq': 100, 'max_bid': 100},
                'enterprise': {'limit': -1, 'price': 49999, 'max_boq': -1, 'max_bid': -1}
            }
            ...
            if existing:
                # ✅ UPDATE existing subscription - but only if it's not already the same plan
                cursor.execute("""
                ...
                print(f"✅ Updated subscription for company {company_id} to {plan}")
            else:
                ...
                print(f"✅ Created new subscription for company {company_id} to {plan}")
